...
const t = {
  ...
  pricing: {
    ...
    cardRight: {
      ...
      cta: "Get free audit",
      trust: "10+ e-shops trust emailflow.uk",
      checklist: [
...
export default function Home() {
  ...
  useEffect(() => {
    const handleIntersect: IntersectionObserverCallback = (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          if (entry.target.id === "pillars") setActiveSection("pillars");
          else if (entry.target.id === "about") setActiveSection("calculator");
          else if (entry.target.id === "workflow") setActiveSection("workflow");
          else if (entry.target.id === "results") setActiveSection("results");
          else if (entry.target.id === "faq") setActiveSection("faq");
        }
      });
    };
    ...
    const sections = ["pillars", "about", "workflow", "results", "faq"];
    sections.forEach((id) => {
    ...
    <div className="bg-surface text-on-surface min-h-screen font-sans selection:bg-emerald-growth/20 overflow-x-hidden">
      ...
      >
        <div className="max-w-container-max mx-auto h-20 px-margin-mobile md:px-margin-desktop flex justify-between items-center">
          <a href="#" className="flex items-center hover:opacity-85 transition-opacity group">
            <span className="text-xl sm:text-[22px] font-display font-bold text-[#6A9AB0] tracking-tight shrink-0">e</span>
            <span className="text-xl sm:text-[22px] font-display font-bold text-deep-navy tracking-tight shrink-0">Laiškai</span>
          </a>
          ...
          <div className="flex items-center gap-4">
            {/* Language Switcher */}
            <div className="hidden sm:flex items-center bg-transparent border border-border-subtle rounded-full p-0.5 shadow-sm">
              <button className="px-3 py-1.5 text-[11px] font-bold bg-[#14483C] text-white rounded-full">LT</button>
              <button className="px-3 py-1.5 text-[11px] font-bold text-deep-navy hover:bg-surface-container rounded-full transition-colors">EN</button>
            </div>

            <a
              href="#audit"
              className="bg-[#0B1E28] hover:bg-[#122F3E] text-white py-2 pl-5 pr-2 rounded-full flex items-center gap-3 hover:scale-95 transition-all text-xs sm:text-[13px] font-bold shrink-0 whitespace-nowrap"
            >
              <span>Rezervuoti konsultaciją</span>
              <div className="w-6 h-6 rounded-full bg-[#C1E6D3] text-[#0B1E28] flex items-center justify-center shrink-0">
        ...
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop flex flex-col md:flex-row justify-between items-center md:items-start gap-8">
          <div className="flex flex-col items-center md:items-start gap-4">
            ...
            <div className="flex flex-col items-center md:items-start gap-2 text-xs text-on-surface-variant font-medium mt-2">
              <a href="mailto:info@emailflow.uk" className="hover:text-emerald-growth transition-colors flex items-center gap-1.5">
